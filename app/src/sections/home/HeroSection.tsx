...
/**
 * Hero — STATIC version (perf hardened).
 * - Removed JS requestAnimationFrame ticker (perpetual main-thread work + forced reflow).
 * - Removed perpetual CSS animations: animate-float / animate-bounce-subtle / animate-pulse.
 * - Ticker is now a pure-CSS marquee using transform (GPU layer, zero JS, zero reflow).
 * - Hover transitions kept (only fire on user input, no idle cost).
 * - prefers-reduced-motion users see a fully still page.
 */
export default function HeroSection() {
  return (
    <section className="relative min-h-screen flex">
      <style>{`
        @keyframes hk-marquee { from { transform: translate3d(0,0,0); } to { transform: translate3d(-50%,0,0); } }
        .hk-marquee-track { animation: hk-marquee 40s linear infinite; will-change: transform; }
        @media (prefers-reduced-motion: reduce) {
          .hk-marquee-track { animation: none !important; }
        }
      `}</style>
      ...
      <div className="hidden lg:block w-1/2 relative bg-gradient-hero overflow-hidden">
        ...
        {/* Phone Mockup — desktop LCP (no animation) */}
          ...
          <div className="relative">
            <picture>
              ...
              <img
                ...
                width="380"
                height="760"
                className="w-[340px] xl:w-[400px] h-auto drop-shadow-2xl rounded-[2.5rem] ring-2 ring-hack-black"
                fetchPriority="high"
                decoding="async"
              />
            </picture>
            {/* Floating Badge - neo-brutal, static */}
            <div className="absolute -bottom-5 -right-6 bg-hack-yellow text-hack-black px-4 py-2 rounded-xl font-display font-bold text-sm border-[3px] border-hack-black shadow-[5px_5px_0_0_#1A1A1A]">
              10K+ Happy Customers
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col justify-center bg-hack-white relative">
        ...
        <div className="relative z-10 px-5 sm:px-8 lg:px-16 xl:px-20 pt-20 pb-32 lg:py-0">
          {/* Tagline pill - neo-brutal (static dot, no pulse) */}
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-hack-yellow rounded-full mb-5 lg:mb-8 border-[2.5px] border-hack-black shadow-[3px_3px_0_0_#1A1A1A]">
            <span className="w-2 h-2 rounded-full bg-hack-black" />
          ...
          {/* Mobile Phone Mockup — MOBILE LCP. WebP only, eager, high priority. (no animation) */}
            ...
            <div className="relative">
              <picture>
                ...
                <img
                  ...
                  width="170"
                  height="340"
                  className="w-[170px] sm:w-[200px] h-auto drop-shadow-2xl rounded-[1.75rem] border-[3px] border-hack-black"
                  fetchPriority="high"
                  decoding="async"
                />
              </picture>
          ...
          {/* CTA Buttons — neo-brutalism (hover transitions kept; idle cost = 0) */}
        ...
        {/* Scroll Ticker — desktop only. CSS-only marquee, GPU-accelerated, zero JS, zero reflow. */}
        <div className="hidden lg:block absolute bottom-16 left-0 right-0 overflow-hidden border-t-[2px] border-b-[2px] border-hack-black py-3 bg-hack-yellow/90">
          <div className="hk-marquee-track flex whitespace-nowrap" style={{ width: "max-content" }}>
            {/* Track is duplicated 2× so the -50% translate keyframe loops seamlessly */}
            {Array.from({ length: 2 }).map((_, dup) => (
              <div key={dup} className="flex whitespace-nowrap">
        ...
        {/* Scroll Down Indicator — neo-brutal (static, no bounce) */}
        <button
          onClick={() =>
            window.scrollTo({
          ...
          className="hidden lg:flex absolute bottom-4 right-6 lg:right-10 w-12 h-12 rounded-xl bg-hack-yellow text-hack-black border-[3px] border-hack-black shadow-[4px_4px_0_0_#1A1A1A] hover:shadow-[2px_2px_0_0_#1A1A1A] hover:translate-x-[2px] hover:translate-y-[2px] transition-all items-center justify-center z-20"
